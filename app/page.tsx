...
import {
  ...
  CheckCircle, Menu, X, Stethoscope, Scissors, PhoneCall,
} from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-dark-950 text-slate-200 overflow-x-hidden">

      {/* ── NAV ── */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-dark-900/95 backdrop-blur-md shadow-sm border-b border-[rgba(59,130,246,0.1)]" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href="#funcionalidades" className="text-sm text-slate-400 hover:text-blue-400 transition-colors">Funcionalidades</a>
            <a href="#precos"          className="text-sm text-slate-400 hover:text-blue-400 transition-colors">Preços</a>
            <a href="#testemunhos"     className="text-sm text-slate-400 hover:text-blue-400 transition-colors">Testemunhos</a>
        ...
        {menuOpen && (
          <div className="md:hidden bg-dark-900/98 border-t border-[rgba(59,130,246,0.1)] px-4 py-4 flex flex-col gap-4">
      ...
      <section className="relative pt-28 pb-20 px-4 sm:px-6 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-500/5 rounded-full -translate-y-1/2 translate-x-1/3" />
          <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-blue-400/5 rounded-full translate-y-1/2 -translate-x-1/3" />
        </div>

        <div className="relative max-w-6xl mx-auto">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm font-semibold px-4 py-2 rounded-full mb-6 animate-fade-in">
      ...
      {/* ── FOR WHO ── */}
      <section className="py-16 px-4 sm:px-6 bg-dark-800 border-y border-[rgba(59,130,246,0.1)]">
        <div className="max-w-6xl mx-auto text-center">
          <p className="text-blue-400 text-sm font-semibold uppercase tracking-widest mb-6">Ideal para</p>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {features.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="card hover:border-blue-500/25 transition-all duration-200 flex gap-5">
      ...
      {/* ── PRICING ── */}
      <section id="precos" className="py-24 px-4 sm:px-6 bg-dark-800">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-blue-400 text-sm font-semibold uppercase tracking-widest mb-3">Preços</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {plans.map((plan) => (
              <div key={plan.name} className={`rounded-2xl p-8 flex flex-col ${plan.highlight ? "bg-blue-500/10 border-2 border-blue-400/40 shadow-lg scale-105" : "bg-dark-700 border border-[rgba(59,130,246,0.1)]"}`}>
                {plan.highlight && (
                  <span className="bg-blue-500 text-white text-xs font-bold px-3 py-1 rounded-full self-start mb-4">MAIS POPULAR</span>
                )}
                <p className="text-slate-500 text-sm mb-1">{plan.name}</p>
                <p className="font-display text-4xl font-bold text-white mb-1">
                  {plan.price} <span className="text-lg font-normal text-slate-500">MZN/mês</span>
                </p>
                <p className="text-slate-600 text-sm mb-6">{plan.desc}</p>
                <ul className="flex flex-col gap-3 mb-8 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm text-slate-400">
                      <CheckCircle className={`w-4 h-4 flex-shrink-0 ${plan.highlight ? "text-blue-400" : "text-slate-600"}`} />
                      {f}
                    </li>
                  ))}
                </ul>
                <Link href="/register" className={`text-center py-3 rounded-xl font-semibold transition-all duration-200 ${plan.highlight ? "bg-blue-500 text-white hover:bg-blue-400" : "border border-[rgba(59,130,246,0.2)] text-slate-300 hover:border-blue-400 hover:text-white"}`}>
                  Começar agora
                </Link>
              </div>
            ))}
      ...
      <section id="testemunhos" className="py-24 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-blue-400 text-sm font-semibold uppercase tracking-widest mb-3">Testemunhos</p>
            <h2 className="font-display text-4xl sm:text-5xl font-bold text-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              <div key={t.name} className="card hover:border-blue-500/20 transition-all duration-200 flex flex-col gap-4">
                ...
                <div className="mt-auto pt-4 border-t border-[rgba(59,130,246,0.08)]">
      ...
      {/* ── CTA ── */}
      <section className="py-24 px-4 sm:px-6 bg-blue-500/5 border-y border-[rgba(59,130,246,0.1)]">
      ...
      {/* ── FOOTER ── */}
      <footer className="bg-dark-900 text-slate-500 py-12 px-4 sm:px-6 border-t border-[rgba(59,130,246,0.08)]">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between gap-8">
          ...
          <div className="flex gap-12 text-sm">
            <div className="flex flex-col gap-2">
              ...
              <Link href="/register"     className="hover:text-blue-400 transition-colors">Registar</Link>
            </div>
            <div className="flex flex-col gap-2">
              <p className="text-white font-semibold mb-1">Legal</p>
              <Link href="/privacidade" className="hover:text-blue-400 transition-colors">Privacidade</Link>
              <Link href="/termos"      className="hover:text-blue-400 transition-colors">Termos</Link>
            </div>
          </div>
        </div>
        <div className="max-w-6xl mx-auto mt-10 pt-6 border-t border-[rgba(59,130,246,0.06)] text-xs text-center">
