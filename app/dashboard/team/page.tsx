    ...
    if (!res.ok) {
      ...
      setInviteSuccess(
        data.userExists
          ...
          : "Convite enviado! O membro precisa de criar conta com este email."
      );
      setInviteEmail("");
      load();
    }
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {maxMembers === -1
              ? "Membros ilimitados"
              : `${activeMembers} de ${maxMembers} membro${maxMembers > 1 ? "s" : ""} activo${maxMembers > 1 ? "s" : ""}`}
          </p>
        </div>
            ...
            {members.map((member) => (
              <div key={member.id} className="px-6 py-4 flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                  member.status === "active" ? "bg-green-100" : "bg-amber-100"
                }`}>
      ...
      <div className="mt-6 bg-purple-50 border border-purple-100 rounded-2xl p-5 text-sm text-purple-800">
        ...
        <p className="text-purple-700 leading-relaxed">
          Os membros convidados podem ver e gerir agendamentos, mas não podem alterar definições do negócio, gerir planos ou remover outros membros. Só o proprietário tem acesso total.
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowModal(false); setInviteError(""); setInviteSuccess(""); }} className="p-2 hover:bg-gray-100 rounded-xl text-gray-500">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {inviteSuccess ? (
              <div className="text-center py-6">
                ...
                <button onClick={() => { setShowModal(false); setInviteSuccess(""); }} className="btn-primary mt-6 mx-auto">
                  Fechar
                </button>
              </div>
            ) : (
              <form onSubmit={handleInvite} className="flex flex-col gap-4">
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-2">
                    Se o membro já tem conta no AgendaMoz, será adicionado imediatamente. Caso contrário, precisará criar conta com este email.
                  </p>
                </div>
